...
void computeWheelsPID(){
  // Параметры PID (подберите их эмпирически)
  const float Kp = 2.0;    // пропорциональный коэффициент
  const float Ki = 2.5;    // интегральный коэффициент
  const float Kd = 0.0;    // дифференциальный коэффициент (можно попробовать небольшое значение, например 0.1)
  const float Kff = 0.3;   // коэффициент feedforward
  // Ограничение интегральной составляющей (anti-windup)
  const float integralLimit = 100.0;

  // Статические переменные для сохранения состояний PID для каждого колеса
  ...
  // Добавляем переменные для отслеживания предыдущих целевых скоростей
  static float lastTargetLeft = 0.0;
  static float lastTargetRight = 0.0;

  // Сброс интегральной составляющей при изменении целевой скорости (anti-windup)
  ...
  // Если dt слишком мал, задаём минимальное значение, чтобы избежать деления на 0
  if(dt < 0.001f) dt = 0.001f;
  lastPIDTime = now;

  // Ошибки для левого и правого колес: разница между требуемой и измеренной скоростью (в мм/с)
  ...
  // Ограничиваем интеграл, чтобы избежать «windup»
  ...
  // Сохраняем текущие ошибки для следующего цикла
  prevErrorLeft  = errorLeft;
  prevErrorRight = errorRight;

  // Вычисляем PID-выход для каждого колеса
  float pidLeft  = Kp * errorLeft  + Ki * errorLeftIntegral  + Kd * derivativeLeft;
  float pidRight = Kp * errorRight + Ki * errorRightIntegral + Kd * derivativeRight;

  // Добавляем feedforward для улучшения динамики управления
  float outputLeft  = pidLeft  + Kff * targetLeftWheelSpeed;
  float outputRight = pidRight + Kff * targetRightWheelSpeed;

  // Преобразуем рассчитанный выход в значения ШИМ для управления направлением и скоростью.
  // Если выход положительный – движение вперёд, если отрицательный – назад.
  ...
  // Передаём рассчитанные значения ШИМ в моторы
